...
export const ProjectCommunityPage: React.FC<{ item?: FeaturedItem }> = ({ item }) => {
    ...
    const fetchAll = async () => {
        ...
        try {
            ...
            const sample = (urlCategory && urlSubcategory ? allProducts.find(p => p.category === urlCategory && p.subcategory === urlSubcategory && p.page_type === 'community' && (targetAgencyId ? p.agency_id?.toString() === targetAgencyId : !p.agency_id)) : null) ||
                          ...
                          (urlCategory && !urlSubcategory ? allProducts.find(p => p.category === urlCategory && p.page_type === 'community' && (targetAgencyId ? p.agency_id?.toString() === targetAgencyId : !p.agency_id)) : null) ||
                          (urlCategory && !urlSubcategory ? allProducts.find(p => p.category === urlCategory && p.page_type === 'community') : null) ||
            ...
            let communityProducts = (agencyProducts.length > 0 ? agencyProducts : allProducts)
                .filter(p => p.page_type === 'community');
