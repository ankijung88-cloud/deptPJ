const fs = require('fs');
const filepath = 'c:/dev/DEPT-Pj2-main/src/pages/AboutPage.tsx';
let content = fs.readFileSync(filepath, 'utf8');

const sText = content.substring(content.indexOf('            {/* 03. Executive Summary */}'), content.indexOf('            {/* 06. Core Theme */}'));
...
const replacement = `            {/* 03. Project Overview */}
...
let newContent = content.replace(sText, replacement);

for (let i = 6; i <= 22; i++) {
    const obId = i.toString().padStart(2, '0');
    const nbId = (i - 2).toString().padStart(2, '0');
    // replace comments
    newContent = newContent.replace(new RegExp(`\\{\\/\\* ${obId}\\.`, 'g'), `{/* ${nbId}.`);
    // replace ids
    newContent = newContent.replace(new RegExp(`id="${obId}"`, 'g'), `id="${nbId}"`);
}
