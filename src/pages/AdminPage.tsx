...
const ProductManager = ({ agencies }: { agencies: any[] }) => {
    ...
    const getNormalizedSubcategoryId = (sub: string) => {
        ...
        // Floor 1: TECH & CARE
        if (['car', 'trend', 'exchange', 'car-care', 'car-care-exchange-week', 'global', '글로벌', 'tech'].includes(s)) return 'car-care';
        if (['window', '디지털쇼윈도', '디지털 쇼윈도'].includes(s)) return 'window';
        
        // Floor 4: GATHER MALL
        if (['talk', 'b2b-mall', '인터뷰', 'interview', 'mall'].includes(s)) return 'b2b-mall';
